#include "Image.h"
unsigned char OV7725_IMAGE[CAMERA_H][CAMERA_W];
////////////////////////////////////////////
/**
* @brief：左边界/右边界/中线
*       ： 定义中间扫描值
...
#define END_ROW 53 //(CAMERA_H - 1 - 6)	//Lock---终止搜索行
...
/**
* @brief:画中线和有效行的截止线
* @param[in]:当前行，特指截止线---CurrentLine 中线建议取0
* @param[in]:选择标志位 0---中线 1---截止线
* @reutrn:状态
*/
Status drawLine(int CurrentLine,int Flag){
	if(0 == Flag){
		for(int i = 0;i < CAMERA_H;i++){
			img(i,CenterGuideGo(i)) = 0;
		}
	}else{
...
Status ImageFinshed(void){
	...
	ProcessImage();
	CenterGuideK = DetectWayType(&_BeginLine, &CenterTLSM);
	...
	LED_PrintValueC(90,0,CenterControl);
	LED_PrintValueC(90,1,_BeginLine.ValidEndLine);
	LED_PrintValueC(90,2,TimeCount);
	LED_PrintValueC(90,3,abs(myround(CenterGuideK)));
	drawLine(0,0);
	drawLine(_BeginLine.ValidEndLine,1);
	LED_PrintImage(OV7725_IMAGE);
	return 'T';
}
////////////////////////////////////////////
//Lock
...
Status GetSteerError(BeginLine *_BeginLine){
	...
	float	CenterSum = 0, CenterSumOther = 0, CenterWeight = 0, CenterData = 0;
	int CenterValidLine = _BeginLine->ValidEndLine;
	static int LastErr = 0;
	//最后一项 195 严重切
	//125 刚刚擦边
	//75 有点空隙过弯道
	int PartWeight[8] = { 0, 1, 5, 5, 75, 125, 175, 80};	//125 195
	//int PartWeight[8] = { 0, 5, 5, 20, 50, 150, 200, 20 };
	//int PartWeightSum[8] = { 0, 5, 5, 25, 75, 225, 245, 255 };
	//int PartWeight[8] = { 0, 0, 5, 20, 75, 150, 20, 5 };
	//int PartWeightSum[8] = { 0, 0, 5, 25, 100, 250, 270, 275 };
	...
	{
		CenterErrGo(n) = CENTER_GUIDE - CenterGuideGo(n);
	}
	/*
	for (int n = 1; n < (EndLine - CenterValidLine); n++){
		if (CenterGuideGo(CenterValidLine + n) < StartCol)
			CenterGuideGo(CenterValidLine + n) = StartCol;
		if (CenterGuideGo(CenterValidLine + n) > EndCol)
			CenterGuideGo(CenterValidLine + n) = EndCol;
	}
	*/
	if (_BeginLine->ValidEndLine > 40)
		PartCount += ((_BeginLine->ValidEndLine - 40) / 5 + 1);
	if (PartCount > 8) PartCount = 8;
	EveryPartNum = (_BeginLine->ValidEndLine - START_ROW) / (PartCount);
	OtherCount = (_BeginLine->ValidEndLine - START_ROW) % EveryPartNum;

	/*统计特性*/
	if (_BeginLine->ValidEndLine == 49
		|| _BeginLine->ValidEndLine == 50){
		OtherCount += PartCount;
	}
	else{
		OtherCount++;
	}
	...
	{
		CenterSumOther += (float)CenterErrGo(i * EveryPartNum + j + START_ROW) * PartWeight[PartCount - 1];
		CenterWeight += PartWeight[PartCount - 1];
	}//分部

	CenterData = (CenterSum + CenterSumOther) / CenterWeight;
	LastErr = CenterData;
...
Status DetectWayType(BeginLine *_BeginLine, TLSM *_TLSM){
	...
	int EveryPart = 0, OtherPart = 0;
	int PartCount = 10;
	int CenterPartX[12] = { 0 };
	int CenterPartY[12] = { 0 };
	EveryPart = (_BeginLine->ValidEndLine - 1) / 10;
	OtherPart = (_BeginLine->ValidEndLine - 1) % 10;
	//没有点不执行该程序
	if (0 == EveryPart && 0 == OtherPart) return 0;
	//不足10个点直接拟合
	if (0 == EveryPart){	
		LinearLine(_TLSM, 1, _BeginLine->ValidEndLine, Center);
	}
	//超过10个采样取点拟合
	else{
		PartCount += OtherPart / EveryPart;
		if (PartCount > 12) PartCount = 12;
		for (int i = 0; i < PartCount; i++){
			CenterPartX[i] = 1 + i*EveryPart + EveryPart / 2 + 1;
			CenterPartY[i] = CenterGuideGo(1 + i*EveryPart + EveryPart / 2 + 1);
		}
		CalculateTLSM(_TLSM, CenterPartX, CenterPartY, PartCount);
	}
	return (int)(_TLSM->k * 100.0f);
}
////////////////////////////////////////////
//Lock               
/**
...
Status ProcessImage(void){
	...
	if (TimeCount >= 99) TimeCount = 99;
	if (TimeCount >= 25 && 0 == StStartLine){
		//===决定是否存在起跑线
		StStartLine =  DetectStartLine1(StVStart, StVEnd,CENTER_GUIDE);
	}
	//===超过1min自动停车
	//if(TimeCount > 60 && 0 == StStartLine){ 
	//	StStartLine = 1;
	//}
	return OK;
}
...
Status SearchObStacleFromMid(int CurrentLine,
	...
	if (None == *SearchFlag){
		...
		if (0 != *ObFirstLine && 0 != *ObEndLine){
			...
			ObActualCenterWidth = 18 - (int)(0.2f * ObCenterDot[0]);
