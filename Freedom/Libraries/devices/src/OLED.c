...
void LED_Set_Pos1(uint8_t x, uint8_t y)//设置坐标函数
{
    LED_WrCmd(0xb0 + y);
    LED_WrCmd(((x & 0xf0) >> 4) | 0x10);
    LED_WrCmd((x & 0x0f) | 0x01);
}
//------------------------------------------------------
//     对全屏写入同一个字符函数
...
//		0			0
//------------------------------------------------------
void LED_Fill(uint8_t bmp_data)
{
    uint8_t y, x;
...
//返回：无
//==============================================================
void LCD_Rectangle(uint8_t x1,uint8_t y1)
{
	uint8_t n; 
...
//==============================================================
//函数名：void LED_P6x8Char(uint8_t x,uint8_t y,uint8_t ch);
...
//==============================================================
//函数名：LED_P6x8Str(uint8_t x,uint8_t y,uint8_t *p)
...
//==============================================================
//函数名：void LED_P8x16Char(uint8_t x,uint8_t y,uint8_t ch);
//功能描述：显示一个6x8标准ASCII字符
//参数：x为显示的横坐标0~122，y为页范围0～7，ch要显示的字符
//返回：无
//==============================================================
void LED_P8x16Char(uint8_t x, uint8_t y, uint8_t ch)
{
    uint8_t c = 0, i = 0;
...
//==============================================================
//函数名：LED_P8x16Str(uint8_t x,uint8_t y,uint8_t *p)
...
//==============================================================
//函数名：LED_P14x16Str(uint8_t x,uint8_t y,uint8_t *p)
...
//==============================================================
//函数名：LED_PXx16MixStr(uint8_t x,uint8_t y,uint8_t *p)
...
//==============================================================
//函数名：  void LED_PrintBMP(uint8_t x0,uint8_t y0,uint8_t x1,uint8_t y1,uint8_t bmp[]);
...
void LED_PrintValueI(uint8_t x, uint8_t y, int data)
{
    ...
    j = (data % 100) / 10;
    k = data % 10;
    LED_P8x16Char(x + 6, y, l + 48);
    LED_P8x16Char(x + 12, y, m + 48);
    LED_P8x16Char(x + 18, y, i + 48);
    LED_P8x16Char(x + 24, y, j + 48);
    LED_P8x16Char(x + 30, y, k + 48);
}

void LED_PrintValueFP(uint8_t x, uint8_t y, uint32_t data, uint8_t num)
{
    uint8_t m, i, j, k;
    LED_P6x8Char(x, y, '.');
    m = data / 1000;
...
void LED_PrintValueF(uint8_t x, uint8_t y, float data, uint8_t num)
{
    ...
    case 0:
        break;
    case 1:
        LED_PrintValueFP(x + databiti * 6, y, (unsigned int)(tempdatalp / 1000), num);
        break;
    case 2:
        LED_PrintValueFP(x + databiti * 6, y, (unsigned int)(tempdatalp / 100), num);
        break;
    case 3:
        LED_PrintValueFP(x + databiti * 6, y, (unsigned int)(tempdatalp / 10), num);
        break;
    case 4:
        LED_PrintValueFP(x + databiti * 6, y, (unsigned int)(tempdatalp), num);
        break;
    }
}
...
/*****************************************************************************
 ...
 返 回 值  : none
*****************************************************************************/
void LED_PrintImage(uint8_t pucTable[][LED_MAX_COLUMN_NUM])
{
    uint8_t href_gap = 1;
    uint8_t cref_gap = 1; //不隔行,隔多少行，就是多少
    uint16_t usRowNum = LED_MAX_ROW_NUM / href_gap, usColumnNum = LED_MAX_COLUMN_NUM / cref_gap;
    uint8_t ucData;
    uint16_t i, j, k, m, n;
    uint16_t usRowTmp;
    ...
    {
        LED_Set_Pos(0, (uint8_t)i);
        usRowTmp = i << 3;    //计算当前所在页的起始行标
        for(j = 0; j < usColumnNum; j++) //列扫描
        {
            ...
            {
                ucData = ucData >> 1;
                if((*(*pucTable + (usRowTmp * href_gap + k * href_gap) * usColumnNum + j * cref_gap)) != LED_IMAGE_BLACK)
                {
                    ucData = ucData | 0x80;
                }
    ...
    LED_Set_Pos(0, (uint8_t)i); //设置剩下的行显示的起始坐标
...
void LED_PrintImageMT(uint8_t pucTable[][LED_MAX_COLUMN_NUMMT], int threshold)
{
    uint8_t href_gap = 1;
    uint8_t cref_gap = 1; //不隔行,隔多少行，就是多少
    uint16_t usRowNum = LED_MAX_ROW_NUMMT / href_gap, usColumnNum = LED_MAX_COLUMN_NUMMT / cref_gap; //60,90
    uint8_t ucData;
    uint16_t i, j, k, m, n;
    uint16_t usRowTmp;
    //int threshold=170;

    m = usRowNum >> 3; //计算图片行数以8位为一组完整的组数 7
    n = usRowNum % 8;  //计算分完组后剩下的行数   4

    for(i = 0; i < m; i++) //完整组行扫描
    {
        LED_Set_Pos(0, (uint8_t)i);
        usRowTmp = i << 3;    //计算当前所在页的起始行标 *8
        for(j = 0; j < usColumnNum; j++) //列扫描
        {
            ucData = 0;
            for(k = 0; k < 8; k++) //在i组中对这8行扫描
            {
                ucData = ucData >> 1;
                if((*(*pucTable + (usRowTmp * href_gap + k * href_gap) * LED_MAX_COLUMN_NUMMT + j * cref_gap)) >= threshold)
                {
                    ucData = ucData | 0x80;
                }
