...
/* Predefined clock setups
   ...
   6 ... Multipurpose Clock Generator (MCG) in Bypassed Low Power External (PEE) mode
         ...
         Core clock = 96MHz, BusClock = 48MHz
	 7 ...  PEE Core 150 Bus 75
	 8 ...  PEE Core 160 Bus 80
	 9 ...  PEE Core 180 Bus 90
*/
...
#define DEFAULT_SYSTEM_CLOCK            150000000u
#elif (CLOCK_SETUP == 8)
#define CPU_XTAL_CLK_HZ                 50000000u
#define CPU_XTAL32k_CLK_HZ              32768u
#define CPU_INT_SLOW_CLK_HZ             32768u
#define CPU_INT_FAST_CLK_HZ             4000000u
#define DEFAULT_SYSTEM_CLOCK            160000000u
#elif (CLOCK_SETUP == 9)
#define CPU_XTAL_CLK_HZ                 50000000u
#define CPU_XTAL32k_CLK_HZ              32768u
#define CPU_INT_SLOW_CLK_HZ             32768u
...
void SystemInit (void)
{
    ...
    while((MCG->S & 0x0Cu) != 0x08u);                   /* 等待 FBE 被选择 */
    MCG->C5 = (uint8_t)MCG_C5_PRDIV0(12);               /* 50/13 */
    MCG->C6 = (uint8_t)(0x40u | MCG_C6_VDIV0(15));      /* 50/13*39 = 150 */
    SIM->CLKDIV1 = (SIM_CLKDIV1_OUTDIV1(0) | SIM_CLKDIV1_OUTDIV2(0) | SIM_CLKDIV1_OUTDIV3(0) | SIM_CLKDIV1_OUTDIV4(7));
    while((MCG->S & MCG_S_PLLST_MASK) == 0u);           /* 等待PLLS 时钟源转到 PLL */
    while((MCG->S & MCG_S_LOCK0_MASK) == 0u);           /* 等待锁定 */
    /* 启动PLL */
    MCG->C1 = (uint8_t)0x1Au;
    ...
    MCG->C6 = (uint8_t)(0x40u | MCG_C6_VDIV0(24));      /* 50/15*48 = 160 */
    SIM->CLKDIV1 = (SIM_CLKDIV1_OUTDIV1(0) | SIM_CLKDIV1_OUTDIV2(1) | SIM_CLKDIV1_OUTDIV3(1) | SIM_CLKDIV1_OUTDIV4(7));
    while((MCG->S & MCG_S_PLLST_MASK) == 0u);           /* 等待PLLS 时钟源转到 PLL */
    while((MCG->S & MCG_S_LOCK0_MASK) == 0u);           /* 等待锁定 */
    /* 启动PLL */
    ...
    while((MCG->S & 0x0Cu) != 0x08u);                   /* 等待 FBE 被选择 */
    MCG->C5 = (uint8_t)MCG_C5_PRDIV0(14);               /* 50/15 */
    MCG->C6 = (uint8_t)(0x40u | MCG_C6_VDIV0(30));      /* 50/15*54 = 180 */
    SIM->CLKDIV1 = (SIM_CLKDIV1_OUTDIV1(0) | SIM_CLKDIV1_OUTDIV2(1) | SIM_CLKDIV1_OUTDIV3(1) | SIM_CLKDIV1_OUTDIV4(7));
    while((MCG->S & MCG_S_PLLST_MASK) == 0u);           /* 等待PLLS 时钟源转到 PLL */
    while((MCG->S & MCG_S_LOCK0_MASK) == 0u);           /* 等待锁定 */
    /* 启动PLL */
