...
**/
#define TLSMLength 20	//最小二乘
#define ForecastTHSMXX(in,a,c)  (int)((in)*(in)*(a)+(c))
#ifdef _MSC_VER
#pragma pack(push,4)
#endif
typedef struct
...
extern unsigned char ov7725_image_show[CAMERA_H][CAMERA_W];
#define img(i,j) ov7725_image[(CAMERA_H - (i) - 1)][(j)]
#define img_show(i,j) ov7725_image_show[(CAMERA_H - (i) - 1)][(j)]
...
Status ProcessImage(void);
int GetSteerError(BeginLine *_BeginLine);
